...
import { Target, ChevronRight, Clock, Image } from 'lucide-react'; // Added Image icon
...
const ICON_COLORS = {
  'Anatomy': 'bg-pink-200', 'Physiology': 'bg-blue-200', 'Pharmacology': 'bg-purple-200', 'Biochemistry': 'bg-teal-200', 'Pathology': 'bg-yellow-200', 'Microbiology': 'bg-green-200', 'Medicine': 'bg-indigo-200', 'Surgery': 'bg-red-200', 'Obstetrics & Gyn': 'bg-pink-300', 'Pediatrics': 'bg-yellow-300', 'Ophthalmology': 'bg-purple-300', 'ENT': 'bg-orange-200', 'Dermatology': 'bg-cyan-200', 'Psychiatry': 'bg-pink-300', 'Orthopedics': 'bg-orange-300', 'Radiology': 'bg-blue-300', 'Anesthesia': 'bg-cyan-300'
};
...
const SubjectTestView = () => {
    ...
    allSubjects
      ...
      .forEach(subj => {
        if (!subjectMap.has(subj.name)) {
          subjectMap.set(subj.name, { name: subj.name, totalQuestions: 0, totalModules: 0, sources: [] });
        }
  ...
  const handleStartSubjectTest = (subjectName) => {
    startQuiz({
      ...
      timer: isStrictTiming ? 50 * 60 : null
    });
  };
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
              ...
              <span className="text-purple-500 font-medium">{Q_BANK_SOURCES.map((src, idx) => <span key={src}>{src}{idx < Q_BANK_SOURCES.length - 1 ? ', ' : '.'}</span>)}</span>
            </p>
          </div>
             ...
             <button
              ...
              >
                ...
                <div className="text-left">
                  <span className="block font-bold text-sm">Images</span>
                  <span className="text-xs opacity-80">{showImageQuestions ? 'Included' : 'Text Only'}</span>
                </div>
              </button>
        ...
        {selectedSources.length === 0 ? (
          <div className={`${CardStyle.bg} ${CardStyle.border} p-8 rounded-xl text-center`}>
            <p className={isDarkMode ? 'text-gray-400' : 'text-gray-600'}>Please select at least one source to view subjects.</p>
          </div>
        ) : metadataLoading ? (
          <div className={`${CardStyle.bg} ${CardStyle.border} p-8 rounded-xl text-center`}><p className={isDarkMode ? 'text-gray-400' : 'text-gray-600'}>Loading subjects...</p></div>
        ) : filteredSubjects.length === 0 ? (
          <div className={`${CardStyle.bg} ${CardStyle.border} p-8 rounded-xl text-center`}><p className="text-amber-500">No subjects found for selected sources.</p></div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredSubjects.map((subject) => (
              <div key={subject.name} className={`${CardStyle.bg} ${CardStyle.border} p-6 rounded-2xl shadow-md hover:shadow-lg transition group`}>
                ...
                <h3 className="text-xl font-bold mb-2" style={{ color: getTextColor() }}>{subject.name}</h3>
                <p className={`text-sm mb-4 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>{subject.totalQuestions} Questions | {subject.totalModules} Modules</p>
                <button onClick={() => handleStartSubjectTest(subject.name)} className="w-full flex items-center justify-between px-4 py-2 rounded-lg font-medium text-purple-600 hover:bg-purple-50 transition group-hover:bg-purple-500 group-hover:text-white">
