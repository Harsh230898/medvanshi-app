...
const QBankView = () => {
  ...
  // CHANGED: Default difficulty is now null (optional/unselected)
  ...
  // NEW: Toggle Difficulty (allows deselecting)
  ...
  const handleStartCustomQuiz = () => {
    ...
    const filters = {
      ...
      difficulty: selectedDifficulty, // Can be null now
      ...
      <div className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-3 gap-2">
              {DIFFICULTY_OPTIONS.map((d) => (
                <button 
                  ...
                  className={`px-2 py-3 rounded-xl font-semibold border-2 text-sm transition-all ${
                    selectedDifficulty === d
                      ? 'bg-green-500 border-green-500 text-white shadow-md'
                      : 'bg-white dark:bg-slate-800 hover:border-green-500'
                  }`}
                >
                  {d}
                </button>
        ...
        <div onClick={() => setIsStrictTiming(!isStrictTiming)} className={`p-4 rounded-xl border-2 cursor-pointer flex items-center justify-between ${isStrictTiming ? 'bg-orange-50 dark:bg-orange-900/20 border-orange-500' : 'bg-white dark:bg-slate-800'}`}>
          <div className="flex items-center gap-3">
             ...
             <div><h3 className="font-bold">Timed Mode</h3><p className="text-xs opacity-70">Auto-submit when time ends. No pause allowed.</p></div>
          </div>
